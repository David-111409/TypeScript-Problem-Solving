...
return the difference between the largest and smallest integers 
in the array.
...
export function difference(ar:number[]):number{
  if (ar.length === 0) return 0;

  let theBig : number = Math.max(...ar);
  let theSmall : number = Math.min(...ar);
  let result = theBig - theSmall;
