...
Calculate only the root that sums the square root of the discriminant, 
not the one that subtracts it.
Round the value / return only integer value.
 */

function quadraticEquation(a:number, b: number, c:number):number{

    let dis = Math.sqrt(Math.pow(b , 2) - 4 * a  * c);
